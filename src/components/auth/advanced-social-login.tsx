...
interface AdvancedSocialLoginProps {
  callbackUrl?: string
  disabled?: boolean
...
export default function AdvancedSocialLogin({ 
  ...
  showLabels = true,
  size = 'md'
}: AdvancedSocialLoginProps) {
  const [loadingProvider, setLoadingProvider] = useState<string | null>(null)

  // Filter providers based on enabledProviders prop
  const providers = enabledProviders 
    ? ALL_PROVIDERS.filter(provider => enabledProviders.includes(provider.id))
    : ALL_PROVIDERS
  ...
  const gridClasses = layout === 'grid' 
    ? `grid grid-cols-${Math.min(providers.length, 3)} gap-3`
    : 'space-y-3'
      ...
      {providers.map((provider) => (
        <button
          ...
          className={`
            ...
            ${layout === 'grid' && !showLabels ? 'aspect-square' : ''}
          `}
        >
          {loadingProvider === provider.id ? (
            ...
            <div className="flex items-center justify-center space-x-2">
              ...
              {showLabels && (
                <span className={layout === 'grid' ? 'hidden sm:inline' : ''}>
